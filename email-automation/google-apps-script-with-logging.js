// ===== EMAIL AUTOMATION WITH LOGGING & QUOTA TRACKING =====
// Full-featured system with menu, logging, and email tracking
...
class EmailLogger {
  ...
  getQuotaStats() {
    ...
    // Apps Script service: API quota limits

    const stats = this.getStats();
    const dailyLimit = 500; // Adjust based on your account type
...
function getSheetList() {
  ...
  return sheets.map(sheet => sheet.getName()).filter(name => name !== "📊 Email Logs");
}
...
function showSendEmailDialog() {
  ...
  let customerOptions = importedData.map((c, i) => `<option value="${i}">${c.firstName} (${c.email}) - ${c.sheetSource}</option>`).join('');
...
function showStatsDashboard() {
  ...
  const html = HtmlService.createHtmlOutput(`
    ...
    <div class="container">
      ...
      <div class="stat-box" style="background-color: #e3f2fd; border: 1px solid #0066cc;">
        <h3 style="margin-top: 0; color: #0066cc;">ℹ️ Email Quota Info</h3>
        <p style="font-size: 12px; color: #333; margin: 5px 0;">
          <strong>Regular Gmail Accounts:</strong> 500 emails/day<br>
          <strong>Google Workspace:</strong> 10,000+ emails/day<br>
          <strong>Reset Time:</strong> Midnight (your timezone)
        </p>
      </div>
    </div>
  `);
...
function showEmailLogs() {
  ...
  SpreadsheetApp.getUi().alert('✅ Email logs sheet created/opened! Check the "📊 Email Logs" tab in your spreadsheet.');
}
